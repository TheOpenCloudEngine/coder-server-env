# ZooKeeper Operator Template

다음의 파일은 `main.tf` 파일일니다.
...
resource "coder_agent" "main" {
  ...
  startup_script = <<-EOT
    ...
    # Go 다운로드 및 압축 해제 (버전은 최신으로 조정 가능)
    wget https://go.dev/dl/go1.22.0.linux-amd64.tar.gz
    sudo rm -rf /usr/local/go && sudo tar -C /usr/local -xzf go1.22.0.linux-amd64.tar.gz

    # 환경 변수 설정 (~/.bashrc 끝에 추가)
    ...
    ########################################################################
    # Operator 개발 시 CRD 매니페스트 생성을 돕는 controller-gen과 kustomize
    ########################################################################
    ...
    # 최신 버전 다운로드 (버전 변수 지정 가능, 예: v1.33.0)
    export OPERATOR_SDK_DL_URL=
